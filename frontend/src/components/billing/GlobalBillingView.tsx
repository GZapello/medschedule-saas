...
export const GlobalBillingView:React.FC<{integration?:boolean}>=({integration=false})=>{
  ...
  return <div className="space-y-5">
    ...
    {integration?<div className="bg-white rounded-2xl border p-6 space-y-4"><h2 className="text-xl font-bold">Integrações · Asaas</h2>
      ...
      {connection?.error && <p className="text-red-700">{connection.error}</p>}<button disabled={busy} onClick={()=>void test()} className="bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700 text-white rounded-xl px-4 py-2 font-bold text-xs shadow-xs cursor-pointer disabled:opacity-50">{busy?'Testando…':'Testar conexão'}</button>
    </div>:<>
      ...
      <form onSubmit={e=>{e.preventDefault();void load();}} className="bg-white rounded-xl border p-4 flex flex-wrap gap-3">
        ...
        <button disabled={busy} className="bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700 text-white rounded-lg px-4 py-2 font-bold text-xs shadow-xs cursor-pointer self-end">{busy?'Carregando…':'Filtrar'}</button>
      </form>
