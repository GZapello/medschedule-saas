  ...
  return <footer className="bg-slate-50 border-t border-slate-200/80 text-slate-600 py-14">
    <div className="max-w-7xl mx-auto px-5 sm:px-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pb-10 mb-10 border-b border-slate-200">
        <a href="/" className="flex items-center gap-3 w-fit"><img src="/brand/zemda-icon-96.webp" alt="" width="36" height="36" loading="lazy" /><span><span className="block text-2xl font-black tracking-tight text-slate-900">Zemda</span><span className="text-[10px] uppercase tracking-widest font-bold text-teal-700">Saúde e Gestão</span></span></a>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-x-7 gap-y-9">
        ...
        <div><h2 className={titleClass}>Módulos</h2><a href="/sistema-para-medicos" className={linkClass} onClick={event => { if (onNavigateSeoPage && !event.ctrlKey && !event.metaKey && !event.shiftKey && !event.altKey && event.button === 0) { event.preventDefault(); onNavigateSeoPage('sistema-para-medicos'); } }}>ZemdaMed</a><a className={linkClass} href="/mapa-corporal-clinico">ZemdaBody · transversal</a>{LANDING_MODULES.filter(m => m.id !== 'med').map(module => <a key={module.id} href={moduleHref(module)} className={linkClass} onClick={event => { if (module.slug && onNavigateSeoPage && !event.ctrlKey && !event.metaKey && !event.shiftKey && !event.altKey && event.button === 0) { event.preventDefault(); onNavigateSeoPage(module.slug); } }}>{module.name}</a>)}</div>
