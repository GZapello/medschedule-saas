...
export const RegistrationProfessionSelect: React.FC<{
  value: string;
  ...
  const list = options && options.length > 0 ? options : REGISTRATION_PROFESSIONS;
  const selected = list.find(option => option.id === value);
  const choose = (index: number) => { onChange(list[index].id); setOpen(false); button.current?.focus(); };
  const show = () => { setActive(Math.max(0, list.findIndex(option => option.id === value))); setOpen(true); };
  ...
  return <div ref={root} className="relative min-w-0 w-full" onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setOpen(false); }}>
    <button ref={button} id="registration-profession" type="button" role="combobox" aria-label="Profissão"
      ...
      onKeyDown={event => {
        ...
        if (['ArrowDown', 'ArrowUp', 'Home', 'End'].includes(event.key)) {
          ...
          setActive(index => event.key === 'Home' ? 0 : event.key === 'End' ? list.length - 1 : Math.max(0, Math.min(list.length - 1, index + (event.key === 'ArrowDown' ? 1 : -1)))); return;
        }
        ...
        if (event.key.length === 1 && !event.ctrlKey && !event.metaKey && !event.altKey) {
          ...
          const index = list.findIndex(option => norm(option.label || option.name || '').startsWith(norm(search.current.text)));
      ...
      {selected?.displayOption || (loading ? 'Carregando profissões...' : 'Selecione sua profissão...')}
    ...
    {open && <div id={id} role="listbox" aria-label="Profissões" className="absolute left-0 right-0 top-full mt-1 z-10 max-h-60 overflow-y-auto overscroll-contain rounded-xl border border-slate-200 bg-white shadow-xl">
      {list.map((option, index) => <div key={option.id} id={`${id}-${index}`} role="option" aria-selected={option.id === value}
        ...
        {option.displayOption || `${option.label || option.name} — ${(option.modules || ['Recursos gerais do Zemda', 'ZemdaBody']).join(' + ')}`}
      </div>)}
    </div>}
  </div>;
};
