...
export const BACKEND_SEO_PAGES: Record<string, SeoPageData> = {
...
export function renderPreRenderedHtml(baseIndexHtml: string, reqPath: string): string {
  ...
  // 3. Verifica se a rota requisitada é uma das páginas de nicho de SEO
  const cleanSlug = reqPath.replace(/^\/+|\/+$/g, '');
  const seoData = BACKEND_SEO_PAGES[cleanSlug];

  if (!seoData) {
    return html;
  }

  // Atualiza metatags de cabeçalho específicas para essa página
  html = html.replace(/<title>.*?<\/title>/i, `<title>${seoData.title}</title>`);
  html = html.replace(/<meta name="description" content=".*?" \/>/i, `<meta name="description" content="${seoData.metaDescription}" />`);
  html = html.replace(/<meta name="keywords" content=".*?" \/>/i, `<meta name="keywords" content="${seoData.keywords}" />`);
  html = html.replace(/<link rel="canonical" href=".*?" \/>/i, `<link rel="canonical" href="https://zemda.com.br${seoData.path}" />`);
  html = html.replace(/<meta property="og:title" content=".*?" \/>/i, `<meta property="og:title" content="${seoData.title}" />`);
  html = html.replace(/<meta property="og:description" content=".*?" \/>/i, `<meta property="og:description" content="${seoData.metaDescription}" />`);
  html = html.replace(/<meta property="og:url" content=".*?" \/>/i, `<meta property="og:url" content="https://zemda.com.br${seoData.path}" />`);
  html = html.replace(/<meta name="twitter:title" content=".*?" \/>/i, `<meta name="twitter:title" content="${seoData.title}" />`);
  html = html.replace(/<meta name="twitter:description" content=".*?" \/>/i, `<meta name="twitter:description" content="${seoData.metaDescription}" />`);

  // Monta conteúdo HTML semântico pré-renderizado no container #root para indexação pelo Googlebot
  ...
  const allNichesLinks = Object.values(BACKEND_SEO_PAGES)
    .map(p => `<li><a href="https://zemda.com.br${p.path}">${p.badge}</a></li>`)
    .join('\n');

  const semanticContent = `
  <header>
    <nav>
      <a href="https://zemda.com.br/">Zemda • Tecnologia em Saúde</a>
      <ul>${allNichesLinks}</ul>
    </nav>
  </header>
  <main>
    <article>
