...
function getPostureModel(): string {
  return (process.env.PERSONAL_POSTURE_AI_MODEL || process.env.GEMINI_MODEL || '').trim();
}
...
export class PersonalPostureAIController {
  ...
  static async analyze(req: Request, res: Response) {
    ...
    const config = getPostureAIConfiguration();
    if (!config.available) {
      res.status(503).json({ error: config.reason || 'IA postural não configurada.' });
      return;
    }
    ...
    if (!Array.isArray(photos) || !photos.length || photos.length > 4 || photos.some(p => !p || typeof p !== 'object') || new Set(photos.map(p => p.view)).size !== photos.length) {
      res.status(400).json({ error: 'Selecione de uma a quatro vistas distintas.' });
      return;
    }
    const attachments = [];
    for (const photo of photos) {
      if (!POSTURE_VIEWS.includes(photo.view) || typeof photo.file_id !== 'string') {
        res.status(400).json({ error: 'Vista inválida.' });
        return;
      }
      ...
      if (!file || !['image/jpeg', 'image/png', 'image/webp'].includes(file.mime_type) || file.file_size > 10 * 1024 * 1024) {
        res.status(400).json({ error: 'Foto indisponível para este aluno.' });
        return;
      }
    ...
    if (active.has(key) || active.size >= 4 || Date.now() - (recent.get(key) || 0) < 60000) {
      res.status(429).json({ error: 'Aguarde um minuto antes de solicitar outra análise.' });
      return;
    }
    ...
    try {
      ...
      const apiKey = getGeminiApiKey();
      const modelName = getPostureModel();
